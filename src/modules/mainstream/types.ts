...
export interface CreateMainstreamEventRequest {
  ...
  matching_slide?: number; // 匹配滑动值(%)，创建者保留的百分比，范围1-100，可选，默认50
...
export interface MainstreamEventResponse {
  ...
  initial_amount: number;          // 初始amount数量 (1 amount = 0.5U)
  yes_pool: string;
  no_pool: string;
  yes_amount: number;              // YES方的amount数量
  no_amount: number;               // NO方的amount数量
  yes_odds: string;
  no_odds: string;
  ...
  deadline: Date;
  matching_slide: number; // 匹配滑动值(%)，创建者保留的百分比
